...
void SceneTutorial::Initialize()
{
	...
	tutorial[0]= new Sprite("Data/Sprite/チュートリアル１.png");
	tutorial[1] = new Sprite("Data/Sprite/チュートリアル２.png");
	tutorial[2] = new Sprite("Data/Sprite/チュートリアル３.png");
	tutorial[3] = new Sprite("Data/Sprite/チュートリアル４.png");
	tutorial[4] = new Sprite("Data/Sprite/チュートリアル５.png");
	tutorial[5] = new Sprite("Data/Sprite/チュートリアル６.png");

	pawn= new Sprite("Data/Sprite/ポーン.png");
	rook = new Sprite("Data/Sprite/ルーク.png");
	bishop = new Sprite("Data/Sprite/ビショップ.png");
	knight = new Sprite("Data/Sprite/ナイト.png");
	queen = new Sprite("Data/Sprite/クイーン.png");
	king = new Sprite("Data/Sprite/キング.png");
...
void SceneTutorial::Render()
{
	...
	case 0:
	case 1:
		tutorial[switch_state]->Render(rc,
			0, 0, 0, 1920, 1080,
			0,
			1, 1, 1, 1);
		break;
	...
	case 4:
	case 5:
		tutorial[switch_state-1]->Render(rc,
			0, 0, 0, 1920, 1080,
			0,
			1, 1, 1, 1);
		break;
	...
	if(explanation=="Pawn")
		pawn-> Render(rc,
			...
			0,
			1, 1, 1, 1);
	else if (explanation == "Rook")
		rook->Render(rc,
			1400, 100, 0, 500, 800,
			0,
			1, 1, 1, 1);
	else if (explanation == "Bishop")
		bishop->Render(rc,
			1400, 100, 0, 500, 800,
			0,
			1, 1, 1, 1);
	else if (explanation == "Knight")
		knight->Render(rc,
			1400, 100, 0, 500, 800,
			0,
			1, 1, 1, 1);
	else if (explanation == "Queen")
		queen->Render(rc,
			1400, 100, 0, 500, 800,
			0,
			1, 1, 1, 1);
	else if (explanation == "King")
		king->Render(rc,
			1400, 100, 0, 500, 800,
			0,
			1, 1, 1, 1);
...
void SceneTutorial::RemovePieceAt(Position pos)
{
...
//マウス座標 → 盤面座標変換
Position SceneTutorial::ScreenToBoard(int screenX, int screenY)
{
	using namespace DirectX;
...
//Piece を盤面座標から取得
Piece* SceneTutorial::FindSlimeAt(Position pos) {
	for (auto p : pieces) {
...
DirectX::XMFLOAT2 SceneTutorial::WorldToScreen(const DirectX::XMFLOAT3& worldPos) const {
	using namespace DirectX;
	Graphics& graphics = Graphics::Instance();
	Camera& camera = Camera::Instance();
...
void SceneTutorial::GenerateHealSpots() {
...
void SceneTutorial::ApplyPersistentEffect(const ActiveEffect& effect)
{
...
bool SceneTutorial::IsTargetPiece(Position pos, const std::string& requiredColor) const
{
...
bool SceneTutorial::ApplyCardEffect(int effectId, Position targetPos)
{
	// 現在のターンの色
...
Position SceneTutorial::FindRandomEmptyCommonSpot() const
{
